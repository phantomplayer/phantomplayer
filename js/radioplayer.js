...
var radioplayer = {
	config_opts: {
		prod_api: {
			stationList: 	'https://static.radioplayer.ca/v1/json/StationList',
			init: 			'https://cookie.radioplayer.ca/cm/',
			search: 		'https://search.radioplayer.ca/qp/v3/search',
			suggest: 		'https://search.radioplayer.ca/qp/v3/suggest',
			onAir: 			'https://search.radioplayer.ca/qp/v3/onair',
			pollOnAir: 		'https://np.radioplayer.ca/qp/v3/onair',
			nowNext: 		'https://np.radioplayer.ca/qp/v3/events',
			onDemand: 		'https://search.radioplayer.ca/qp/v3/oditem',
			recommend: 		'https://search.radioplayer.ca/qp/v3/recommend',
			az: 			'https://search.radioplayer.ca/qp/v3/stations'
		},
		dev_api: {
	...
	consts: {
		...
		use_global_cookie_settings:	false, // if true, cookie notice will not show if it has been seen before on any other radioplayer player
		cookie_anno_ttl:			(60*60*24*365*10), // 10 years
		api: {},
		iframe: {
			analytics: 		'https://static.radioplayer.ca/v3/analytics.html'
		}
	},
...
radioplayer.services = {
  ...
  cookieInterval: null,
	...
	alerts: {
		...
		init : function() {
			...
			if (adsWizzBlocker || (vastAds.enabled && !vastAds.shownVideo) || $('body').hasClass('showing-overlay')) {
				// We need to wait
		...
		cookies: {
			...
			show: function() {
				...
				// If we have not seen the message, or want to force showing, continue
				if ((radioplayer.consts.show_cookie_anno && document.cookie.indexOf("rp-seen-cookie-anno") == -1) || radioplayer.consts.show_cookie_demo) {
  ...
  overlay : {
  	...
  	queue: function(url, mute) {
  		...
  		if (adsWizzBlocker || (vastAds.enabled && !vastAds.shownVideo) || radioplayer.services.overlay.showing) {
  	...
  	vast: function(playerHTML) {
      	...
      	radioplayer.services.overlay.volumeControl.onOpen(true);
  		}
  	},
  	hide : function() {
  ...
  waitingCount: 0,
	...
	setAudioParams : function(audioHTML, audioLive, bufferTime) {
		...
		if(this.audioHTML.length && this.audioHTML[0].audioType && this.audioHTML[0].audioType === 'httpmp4m4a') {
			...
			if (indexTest > -1) {
				this.audioHTML[0].audioUrl = this.audioHTML[0].audioUrl.substr(0,indexTest);
			}
		}
		this.aborted = false;
		this.audioLive = audioLive;
...
radioplayer.controls = {
	...
	startPlayingInit : function(that) {
		if (vastAds && vastAds.enabled && !vastAds.shownVideo) {
			if(audioLive){
				radioplayer.emp.stop();
			}
			else {
...
radioplayer.adswizz = {
  ...
  parseMetadata: function (x, event) {
		if ((radioplayer.adswizz.enabled) && (event.type == 'metadata')) {
          ...
          //url = "http://placekitten.com/360/517";
...
radioplayer.lang = {
	...
	general: {
		...
		reduced_func_anno:		'You\'re not getting the full Radioplayer experience in this browser. ' + 
								'<a href="http://www.radioplayer.ca/cookiehelp/?mob=false" target="_blank">Change cookie settings</a>, or <a href="http://www.radioplayer.ca/apps" target="_blank">upgrade to our app.</a>',

	    cookie_anno:            'Radioplayer uses cookies to enhance your experience.<br />' +
	                            'To find out how, <a href="http://www.radioplayer.ca/" target="_blank">click here</a>. ' +
	                            'If you continue, we\'ll assume you\'re happy to receive all Radioplayer cookies.',

      cookie_message:          'These features, and others, are powered by <a href="http://www.radioplayer.ca/cookies" target="_blank">cookies</a>. Keep listening if this is OK.',
	...
	search: {
		...
		search_radioplayer: 	'Search Canada Radio',
	...
	stream_error: {
		unavailable: 			'<h1>The sound of silence</h1>' +
								...
								'<p>You can try <a href="https://get.adobe.com/flashplayer/" target="_blank">updating Flash</a> or using a different browser but if your device doesn\'t support this, why not <a href="http://www.radioplayer.ca/apps" target="_blank">download our free app</a> for phones and tablet.</p>',
